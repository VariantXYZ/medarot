...
#TODO: Redo this whole thing, right now it's rather buggy and won't even look at the nested asm
...
download: $(TEXT) cleanadditional
	$(PYTHON) scripts/preparation/get_text.py
...
$(BUILD)/%.$(BIN_TYPE): $(TEXT)/%.$(TEXT_TYPE_LIST)
	$(PYTHON) scripts/preparation/textpre.py list < $(TEXT)/$(*F).$(TEXT_TYPE_LIST) > $(BUILD)/$(*F).$(BIN_TYPE)
...
$(BUILD_ADDITIONAL)/dummy:
	...
	python scripts/preparation/autopad.py 0x4000 $(BUILD_ADDITIONAL)/*.$(BIN_TYPE)
